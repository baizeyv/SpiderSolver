...
std::vector<State *> Solver::take_a_step(State *state, Solver *solver) {
    ...
    for (int i = 0; i < state->visibleCards.size(); i++) {
...
std::vector<State *> Solver::move_movable_cards(const std::vector<Card *> &movableCards,
    ...
    for (int column = 0; column < state->visibleCards.size(); column++) {
        ...
        if (state->is_blank(column)) {
            // # 当前状态的column目标列为空
            if (state->visibleCards[fromIndex].size() + state->hiddenCards[fromIndex].size() == movableCards.size()) {
                // # 全移动的情况,移动后为空列,目标列也是空列
                continue;
            }
            ...
            // # 目标列不为空
            for (int i = movableCards.size(); i >= 1; i--) {
                std::vector<Card *> cards(movableCards.begin(), movableCards.begin() + i);
                if (state->visibleCards[column][0]->value == cards.back()->value + 1) {
                    // # 可以放到目标列 (符合差值为1的条件)

                    // # 这里分为以下几种情况,进行剪枝
                    // * 1. 移动红桃, 红桃上是(其他色成顺|同色不成顺), 移动到方片 (这种情况应该保留)
                    // * 2. 移动红桃, 红桃上是(其他色成顺|同色不成顺), 移动到红桃 (这种情况应该保留)
                    // * 3. 移动红桃, 红桃上是红桃(成顺), 移动到红桃 (这种情况需要判断移动后的顺子长度,应该大于移动前的整个顺子的长度)
                    // * 4. 移动红桃, 红桃上是红桃(不成顺),移动到红桃 (这种情况应该保留)
                    // * 5. 移动红桃, 红桃上是红桃(成顺), 移动到方片 (这种情况应该剪枝)
                    // * 6. 移动红桃, 红桃上是红桃(不成顺),移动到方片 (这种情况应该保留)
                    // * 7. 只要是把movable的所有都移动的情况,不需要剪枝,除非是 红桃1,2,3,4, 将12, 移动到红桃3,4 这种情况需要剪枝

                    // # 1,2两种情况相当于移动所有的 movableCards,可以合并为同一种过滤器

                    // # 先处理情况7
                    if (i == movableCards.size()) {
                        // # 全部移动的情况
                        if (state->visibleCards[fromIndex].size() > i) {
                            // # 还存在覆盖牌
                            const auto movable_last = movableCards.back();
                            const auto static_first = state->visibleCards[fromIndex][movableCards.size()];
                            if (static_first->value - 1 == movable_last->value && static_first->suit == movable_last->
                                suit) {
                                // # 剪枝
                                continue;
                            }
                        }
                        // # 这里就是上述的情况1,情况2,情况4,情况6
                    } else {
                        // # 处理情况 3,5
                        if (state->visibleCards[column].front()->suit == movableCards.front()->suit) {
                            // # 移动到同色,需要判断移动后的顺子长度,应该大于移动前的整个顺子的长度
                            auto back_card = movableCards[i - 1];
                            int will_count = i;
                            for (int x = 0; x < state->visibleCards[column].size(); x++) {
                                if (state->visibleCards[column][x]->suit == back_card->suit && state->visibleCards[
                                        column][x]->value - 1 == back_card->value) {
                                    // # 同色,值差1
                                    will_count++;
                                    back_card = state->visibleCards[column][x];
                                } else {
                                    break;
                                }
                            }
                            if (will_count <= movableCards.size()) {
                                // # 移动后的长度小于当前长度,直接剪枝
                                continue;
                            }
                        } else {
...
std::vector<State *> Solver::sort(std::unordered_set<State *, StatePtrHash, StatePtrEqual> states) {
    ...
    std::ranges::stable_sort(list, [](State *a, State *b) {
        ...
        const int av = a->evaluate();
        const int bv = b->evaluate();
        // return av > bv;
        if (av > bv)
            return true;
        if (av < bv)
